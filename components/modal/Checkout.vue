...
export default {
	...
	computed: {
			products () {
				return this.$store.getters.productsAddedByEmpresa(this.$store.state.empresa.id);
			},
			openModal () {
			...
			buyLabel (eid) {
				let totalProducts = this.products.length,
						productsAdded = this.$store.getters.productsAddedByEmpresa(this.$store.state.empresa.id),
						pricesArray = [],
						productLabel = '',
						finalPrice = '',
						quantity = 1;
	...
	methods: {
		...
		onNextBtn () {
			const tel = this.$store.state.empresa.whatsapp;
			const url = tel+"?text=Oi, gostaria de fazer um pedido"
			window.open(url);
			console.log("Comprar");
		},
		onPrevBtn () {
